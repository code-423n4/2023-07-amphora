...
const packageName = `@amphora-protocol/interfaces`;
const packageDescription = `Amphora Protocol Interfaces and needed integration files`;
const outDir = "./out";
const exportDir = "./npm";
...
glob(interfacesGlob, (err, interfacePaths) => {
  ...
  for (let interfacePath of interfacePaths) {
    ...
    const relativeInterfaceFile = transformRemappings(
      interfaceFile,
      interfacePath,
      remappings
    );
    ...
    fse.outputFileSync(
      path.join(contractsDir, contractPath),
      relativeInterfaceFile
    );

    // get the interface name
    const interface = interfacePath.substring(
      interfacePath.lastIndexOf("/") + 1,
      interfacePath.lastIndexOf(".")
    );

    // copy interface abi to the export directory
    fse.copySync(
      `${outDir}/${interface}.sol/${interface}.json`,
      `${abiDir}/${interface}.json`
    );
  }
  ...
  for (const target of targets) {
    ...
    execSync(
      `yarn typechain --target ${target} --out-dir ${exportDir}/${target} '${abiDir}/*.json'`
    );
  }
});
...
function transformRemappings(file, filePath, remappings) {
  ...
  return fileLines
    .map((line) => {
      ...
      const dependencyDirectory = filePath.replace(
        /solidity.*/,
        `${remappingDestination}`
      );
      const sourceDirectory = filePath
        .split("/")
        .slice(0, -1)
        .join("/");
      ...
      if (line.includes(modulesKey)) {
        ...
        line =
          importPart +
          line.substring(line.indexOf(modulesKey) + modulesKey.length);
      }
